...
# Directories (Changed for GraphCodeBERT)
BASE_DIR = "/content/graphcodebert_output"
OUTPUT_DIR = BASE_DIR
...
# GraphCodeBERT limit is 512 tokens
MAX_LEN = 512

# --- A100 OPTIMIZATIONS ---
# 80GB VRAM allows massive batches.
...
class CodeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        try:
            ...
            # GraphCodeBERT processes code as sequence text here
...
# ==========================================
def main():
    logger.info(f"--- Starting A100 OPTIMIZED Training Job for GraphCodeBERT ---")
    ...
    training_args = TrainingArguments(
        ...
        learning_rate=2e-5,
        per_device_train_batch_size=TRAIN_BATCH_SIZE, # 128
        per_device_eval_batch_size=EVAL_BATCH_SIZE,   # 256
        gradient_checkpointing=False,  # Disabled for speed (A100 has plenty of RAM)
        gradient_accumulation_steps=1, 
        bf16=True,                     # Native A100 support
        dataloader_num_workers=16,     # Feed data fast
        num_train_epochs=3,
        weight_decay=0.01,
        eval_strategy="epoch",
        save_strategy="epoch",
    ...
    # 5. Train & Save
    logger.info(f"Starting Training (Batch Size: {TRAIN_BATCH_SIZE})...")
    trainer.train()
