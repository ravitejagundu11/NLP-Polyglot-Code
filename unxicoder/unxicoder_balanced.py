...
BASE_DIR = "/content/unixcoder_output"
OUTPUT_DIR = BASE_DIR
...
# ==========================================
def main():
    print(f"--- Starting A100 OPTIMIZED Training Job for UniXcoder ---")

    # 1. Load Pre-Prepared Data (Fast Loading)
    ...
    # 3. Create Datasets
    # Assumes columns are 'code' and 'label' as saved by prepare_data.py
    ...
    # 5. Train & Save
    print(f"Starting Training on A100 (Batch Size: {TRAIN_BATCH_SIZE})...")
    trainer.train()
    ...
    # We still load the Test set from HF because we didn't save it locally
    try:
        ...
        print("xxx FINAL TEST RESULTS xxx")
        print(f"Test Accuracy: {test_results.metrics['test_accuracy']:.4f}")
        print(f"Test Macro F1: {test_results.metrics['test_f1_macro']:.4f}")
